...
### 4. **前日の日記とRAGコンテキストの統合** 🆕
日記生成時に以下のコンテキストを自動的に含めます：
- **Yesterday's Journal**: 前日の振り返りセクション（"Tomorrow's Focus"との整合性確認）
- **Historical Insights**: MemoryManagerから取得した過去の洞察（最大3件）
  - タイムスタンプフィルター付き（今日より前のデータのみ）
  - 生産性、集中、達成、課題などのキーワードで検索
...
### 5. **Tomorrow's Focus**
今日の振り返りと前日の目標から、明日への具体的なアクション提案
